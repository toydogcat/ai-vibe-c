# AI Vibe C step06-deploy-firebase - 前端佈署至 Firebase Hosting
...
- `firebase.json` 中的 `hosting.site` 是 Hosting 站點 ID（本例：`<your-hosting-site-id>`）。
- `.firebaserc` 中的 `default` 是 Firebase project ID（本例：`<your-firebase-project-id>`）。
...
npm run build
cd ..
firebase deploy --only hosting:<your-hosting-site-id>
```
